...
def update_team_captain(team_id: int, body: SetCaptainRequest, db: Session = Depends(get_db), room_id: int = Depends(get_active_room_id), user: CurrentUser = Depends(get_current_user)):
    return set_captain(team_id=team_id, player_id=body.player_id, db=db, room_id=room_id, actor_username=user.username)
